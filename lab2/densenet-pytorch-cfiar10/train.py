...
def draw_curve(list_losses, list_accuracy):
    import numpy as np
    import matplotlib.pyplot as plt
    plt.figure(figsize=(8,6))
    plt.plot(np.arange(0,len(list_losses)),list_losses)
    plt.title('loss')
    plt.savefig('loss.png')

    plt.figure(figsize=(8,6))
    plt.plot(np.arange(0,len(list_accuracy)),list_accuracy)
    plt.title('Accuracy')
...
def main():
    ...
    for epoch in range(last_epoch + 1, config.epochs):
        ...
        if (
            ...
            test_accuracy = test(test_loader, net, criterion, optimizer, epoch, device)
    ...
    draw_curve(list_losses, list_accuracy)
